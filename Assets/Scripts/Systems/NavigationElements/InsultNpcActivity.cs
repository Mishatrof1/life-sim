...
{
    public class InsultNpcActivity : IEcsInitSystem, INavigationElement
    {
        ...
        private InsultsSettings _insultsSettings;
        private ReactionsSettings _reactionsSettings;

        public List<NavigationElementType> Types => new List<NavigationElementType> { NavigationElementType.InsultInteraction };
        ...
        {
            if (elementType != NavigationElementType.InsultInteraction)
            {
                return false;
            }
            ...
            foreach (var i in _characterFilter)
            {
                _world.NewEntity()
                    .Replace(new Components.NpcCommunication
                    {
                        Npc = npc,
                        Character = _characterFilter.Get1(i).Character,
                        Communication = new InsultsCommunication(_insultsSettings, _reactionsSettings)
                    })
                    .Replace(new New());
            }
            return true;
