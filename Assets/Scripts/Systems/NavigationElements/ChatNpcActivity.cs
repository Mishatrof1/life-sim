...
{
    public class ChatNpcActivity : IEcsInitSystem, INavigationElement
    {
        ...
        public List<NavigationElementType> Types => new List<NavigationElementType> { NavigationElementType.ChatInteraction };
        ...
        {
            if (elementType != NavigationElementType.ChatInteraction)
            {
                return false;
            }
        ...
        {
            // var npc = _navigationActiveFilter.GetLastElementInChain<Npc>(NavigationBlockType.Main);
            // _world.NewEntity().Replace(new InkStoryComponent { StoryName = "ChatInkStory", CurrentNpc = npc });
            // return true;
            return false;
        }
