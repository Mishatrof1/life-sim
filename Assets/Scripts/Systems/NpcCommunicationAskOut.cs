...
namespace Systems
{
    public class NpcCommunicationAskOut : IEcsRunSystem, IEcsInitSystem, IEcsDestroySystem
    {
        ...
        public void Run()
        {
            foreach (var i in _communicationFilter)
            {
                ...
                if (!(communication.Communication is AskOutCommunication))
                    continue;
                
                if (_navigationFilter.GetCurrentPoint(NavigationBlockType.Main)?.Type != NavigationElementType.AskOutInteraction)
                {
                    entity.Destroy();
                    continue;
                }
