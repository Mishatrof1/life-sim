...
namespace Systems
{
    public class NpcCommunicationBefriend : IEcsRunSystem, IEcsInitSystem, IEcsDestroySystem
    {
        ...
        public void Run()
        {
            foreach (var i in _communicationFilter)
            {
                ...
                if (!(communication.Communication is BefriendCommunication))
                    continue;
                
                if (_navigationFilter.GetCurrentPoint(NavigationBlockType.Main)?.Type != NavigationElementType.BeFriendInteraction)
                {
                    entity.Destroy();
                    continue;
                }
